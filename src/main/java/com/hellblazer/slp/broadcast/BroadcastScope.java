...
/**
 * A service discovery scope based on a broadcast based replication service
 * 
 * @author <a href="mailto:hal.hildebrand@gmail.com">Hal Hildebrand</a>
 * 
 */
public class BroadcastScope implements ServiceScope {
    ...
    private final static Logger log                     = LoggerFactory.getLogger(BroadcastScope.class);
    ...
    public BroadcastScope(Engine engine) {
        this(engine, 2);
    }

    public BroadcastScope(Engine engine, Executor execService) {
        this.engine = engine;
        executor = execService;
    }

    public BroadcastScope(Engine engine, int notificationThreads) {
        this(engine, Executors.newFixedThreadPool(notificationThreads,
                                                  new ThreadFactory() {
                                                      int i = 0;
    ...
    public UUID register(ServiceURL url, Map<String, String> properties) {
        ...
        UUID registration = engine.register(serialize(url, properties,
                                                      engine.getMaxStateSize()));
        if (properties == null) {
    ...
    public void setProperties(UUID serviceRegistration,
        ...
        engine.update(serviceRegistration,
    ...
    public void unregister(UUID serviceRegistration) {
        ...
        if (ref != null) {
            engine.deregister(serviceRegistration);
